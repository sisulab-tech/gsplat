...
rasterize_to_pixels_3dgs_geom_fwd(
    ...
    at::Tensor dist_accums =
        at::empty({C, image_height, image_width, 2}, means2d.options());

#define __LAUNCH_KERNEL__(N)                                                   \
    case N:                                                                    \
        launch_rasterize_to_pixels_3dgs_geom_fwd_kernel<N>(                    \
            means2d,                                                           \
            conics,                                                            \
            colors,                                                            \
            opacities,                                                         \
            ray_planes,                                                        \
            Ks,                                                                \
            static_cast<float>(distort_near),                                  \
            static_cast<float>(distort_far),                                   \
            backgrounds,                                                       \
            masks,                                                             \
            image_width,                                                       \
            image_height,                                                      \
...
rasterize_to_pixels_3dgs_geom_bwd(
    ...
    CHECK_INPUT(dist_accums);
    CHECK_INPUT(v_render_colors);
    CHECK_INPUT(v_render_alphas);
    CHECK_INPUT(v_render_edepths);
    CHECK_INPUT(v_render_mdepths);
    CHECK_INPUT(v_render_distorts);
    if (backgrounds.has_value()) {
        CHECK_INPUT(backgrounds.value());
    }
    if (masks.has_value()) {
...
#define __LAUNCH_KERNEL__(N)                                                   \
    case N:                                                                    \
        launch_rasterize_to_pixels_3dgs_geom_bwd_kernel<N>(                    \
            means2d,                                                           \
            conics,                                                            \
            colors,                                                            \
            opacities,                                                         \
...
#undef __LAUNCH_KERNEL__
    ...
    return std::make_tuple(
        v_means2d_abs, v_means2d, v_conics, v_colors, v_opacities, v_ray_planes
    );
}
...
rasterize_to_pixels_eval3d_geom_fwd(
    ...
    at::Tensor renders = at::empty(
        {C, image_height, image_width, channels}, view2gaussians.options()
    );
    at::Tensor alphas =
        at::empty({C, image_height, image_width, 1}, view2gaussians.options());
    at::Tensor last_ids = at::empty(
        {C, image_height, image_width}, view2gaussians.options().dtype(at::kInt)
    );
    at::Tensor render_normals =
        at::empty({C, image_height, image_width, 3}, view2gaussians.options());
    at::Tensor render_edepths =
        at::empty({C, image_height, image_width, 1}, view2gaussians.options());
    at::Tensor render_mdepths =
        at::empty({C, image_height, image_width, 1}, view2gaussians.options());
    ...
    at::Tensor render_distorts =
        at::empty({C, image_height, image_width, 1}, view2gaussians.options());
    at::Tensor dist_accums =
        at::empty({C, image_height, image_width, 2}, view2gaussians.options());

#define __LAUNCH_KERNEL__(N)                                                   \
    case N:                                                                    \
        launch_rasterize_to_pixels_eval3d_geom_fwd_kernel<N>(                  \
            view2gaussians,                                                    \
            colors,                                                            \
            opacities,                                                         \
            Ks,                                                                \
...
rasterize_to_pixels_eval3d_geom_bwd(
    ...
    at::Tensor v_view2gaussians = at::zeros_like(view2gaussians);
    at::Tensor v_colors = at::zeros_like(colors);
    at::Tensor v_opacities = at::zeros_like(opacities);
    at::Tensor v_means2d = at::zeros_like(means2d);
    at::Tensor v_means2d_abs = at::zeros_like(means2d);

#define __LAUNCH_KERNEL__(N)                                                   \
    case N:                                                                    \
        launch_rasterize_to_pixels_eval3d_geom_bwd_kernel<N>(                  \
            view2gaussians,                                                    \
            means2d,                                                           \
            conics,                                                            \
            colors,                                                            \
...
#undef __LAUNCH_KERNEL__
    ...
    return std::make_tuple(
        v_view2gaussians, v_colors, v_opacities, v_means2d, v_means2d_abs
    );
}
