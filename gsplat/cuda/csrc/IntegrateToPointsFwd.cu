 ...
 * Current implementation is the same as GOF but it could be potentially better
 *parallelized if we distribute the points uniformly instead of using the thread
 *the it projected into
 ****************************************************************************/
...
__global__ void integrate_to_points_fwd_kernel(
    ...
    const uint32_t PN,
    const uint32_t point_n_isects,
    const bool packed,
    const vec2 *__restrict__ points2d,         // [C, N, 2] or [nnz, 2]
    const scalar_t *__restrict__ point_depths, // [C, N, 3] or [nnz, 3]
    ...
    const vec10 *__restrict__ view2gaussians,  // [C, N, 10] or [nnz, 10]
    ...
    const int32_t
        *__restrict__ point_tile_offsets, // [C, tile_height, tile_width]
    ...
    scalar_t *__restrict__ out_integrated_colors, // [C, PN, 3]
    scalar_t *__restrict__ out_integrated_alphas, // [C, PN]
    ...
    render_colors += camera_id * image_height * image_width * (CDIM + 1 + 3);
    render_alphas += camera_id * image_height * image_width;
    out_integrated_alphas += camera_id * PN;
    out_integrated_colors += camera_id * PN * CDIM;
    ...
    scalar_t px = (scalar_t)j + 0.5f;
    scalar_t py = (scalar_t)i + 0.5f;
    int32_t pix_id = i * image_width + j;

    const scalar_t focal_x = Ks[0];
    const scalar_t focal_y = Ks[4];
    const scalar_t cx = Ks[2];
    const scalar_t cy = Ks[5];
    const vec3 ray = {(px - cx) / focal_x, (py - cy) / focal_y, 1.0};
    ...
    uint32_t point_num_batches =
        (point_range_end - point_range_start + block_size - 1) / block_size;
    ...
    vec10 *view2gaussian_batch =
        reinterpret_cast<vec10 *>(&conic_batch[block_size]); // [block_size]
    vec3 *points2d_depth_batch =
        reinterpret_cast<vec3 *>(&view2gaussian_batch[block_size]
        ); // [block_size]
    ...
    scalar_t pix_out[CDIM + 1 + 3] = {0.f};

#define MAX_NUM_CONTRIBUTORS 256

    uint32_t n_contrib_local = 0;
    uint16_t contributed_ids[MAX_NUM_CONTRIBUTORS * 4] = {0};
    // use 4 additional corner points so that we have more accurate estimation
    // of contributed_ids
    scalar_t corner_Ts[5] = {1.0f, 1.0f, 1.0f, 1.0f, 1.0f};
    scalar_t offset_xs[5] = {0.0f, -0.5f, 0.5f, -0.5f, 0.5f};
    scalar_t offset_ys[5] = {0.0f, -0.5f, -0.5f, 0.5f, 0.5f};
    ...
    for (uint32_t b = 0; b < num_batches; ++b) {
        ...
        for (uint32_t t = 0; (t < batch_size) && !done; ++t) {
            ...
            const scalar_t opac = xy_opac.z;
            // why not use pointer so we don't need to copy again?
            const vec10 view2gaussian = view2gaussian_batch[t];

            bool used = false;
            for (uint32_t k = 0; k < 5; ++k) {
                const vec3 ray_k = {
                    (px + offset_xs[k] - cx) / focal_x,
                    (py + offset_ys[k] - cy) / focal_y,
                    1.0
                };

                const vec3 normal = {
                    view2gaussian[0] * ray_k.x + view2gaussian[1] * ray_k.y +
                        view2gaussian[2],
                    view2gaussian[1] * ray_k.x + view2gaussian[3] * ray_k.y +
                        view2gaussian[4],
                    view2gaussian[2] * ray_k.x + view2gaussian[4] * ray_k.y +
                        view2gaussian[5]
                };

                // use AA, BB, CC so that the name is unique
                scalar_t AA =
                    ray_k.x * normal[0] + ray_k.y * normal[1] + normal[2];
                scalar_t BB =
                    2 * (view2gaussian[6] * ray_k.x +
                         view2gaussian[7] * ray_k.y + view2gaussian[8]);
                scalar_t CC = view2gaussian[9];

                // t is the depth of the gaussian
                scalar_t depth = -BB / (2 * AA);

// TODO take near plane as input
#define NEAR_PLANE 0.01f
                // depth must be positive otherwise it is not valid and we skip
                // it
                if (depth <= NEAR_PLANE)
                    continue;

                // the scale of the gaussian is 1.f / sqrt(AA)
                scalar_t min_value = -(BB / AA) * (BB / 4.) + CC;

                scalar_t power = -0.5f * min_value;
                if (power > 0.0f) {
                    power = 0.0f;
                }

                scalar_t alpha = min(0.999f, opac * exp(power));

                if (alpha < 1.f / 255.f) {
                    continue;
                }

                const scalar_t next_T = corner_Ts[k] * (1.0f - alpha);
                ...
                const scalar_t vis = alpha * corner_Ts[k];
                if (k == 0) {
                    const scalar_t *c_ptr = colors + g * CDIM;
#pragma unroll
            ...
            if (used) {
                ...
                if (n_contrib_local >= MAX_NUM_CONTRIBUTORS * 4) {
                    ...
                    printf(
                        "ERROR: Maximal contributors are met. This should be "
                        "fixed! %d\n",
                        n_contrib_local
                    );
                    break;
...
#pragma unroll
        for (uint32_t k = 0; k < CDIM; ++k) {
            render_colors[pix_id * (CDIM + 1 + 3) + k] =
...
#define MAX_NUM_PROJECTED 256
    ...
    // TODO add a for loop here in case we got more points than
    // MAX_NUM_PROJECTED
    ...
    while (true) {
        ...
        for (uint32_t b = 0; b < point_num_batches; ++b) {
            ...
            if (idx < point_range_end) {
                int32_t g =
                    point_flatten_ids[idx]; // flatten index in [C * N] or [nnz]
                id_batch[tr] = g;
                const vec2 xy = points2d[g];
                const scalar_t depth = point_depths[g];
            ...
            // process gaussians in the current batch for this pixel
            uint32_t batch_size =
                min(block_size, point_range_end - point_batch_start);
            for (uint32_t t = 0; (t < batch_size) && !done; ++t) {
                point_counter++;
                if (point_counter <= point_counter_last) {
                    continue;
                }
                const vec3 point_xy_depth = points2d_depth_batch[t];
                ...
                if ((point_xy.x >= (px - 0.5)) && (point_xy.x < (px + 0.5)) &&
                    ...
                    // if (pix_id == 550 * 1237 + 600){
                    //     printf("projected_points_ids: %d points_xy_depth:
                    //     %.10f %.10f %.10f\n", id_batch[t], point_xy.x,
                    //     point_xy.y, depth);
                    // }
                    // ((points2d[0, :, 0:1] >= 600) & (points2d[0, :, 0:1] <=
                    // 601) & (points2d[0, :, 1:2] >= 401) & (points2d[0, :,
                    // 1:2] <=402) ).float().argmax()
                }
            }
        }
        // return;
        ...
        for (uint32_t b = 0; b < num_batches; ++b) {
            ...
            if (idx < range_end) {
                int32_t g =
                    flatten_ids[idx]; // flatten index in [C * N] or [nnz]
                id_batch[tr] = g;
                const vec2 xy = means2d[g];
                const scalar_t opac = opacities[g];
                xy_opacity_batch[tr] = {xy.x, xy.y, opac};
            ...
            for (uint32_t t = 0; (t < batch_size) && !done; ++t) {
                ...
                if (cur_idx !=
                    (uint32_t)contributed_ids[num_contributed_second]) {
                    continue;
                } else {
                ...
                for (uint32_t k = 0; k < num_projected; ++k) {
                    // if (pix_id == 550 * 1237 + 600 && k == 0){
                    //     printf("ealpha last_contributor: %d num_iterated:
                    //     %d\n", batch_start + t - range_start, cur_idx);
                    // }

                    const vec3 ray_k = {
                        (projected_xy[k].x - cx) / focal_x,
                        (projected_xy[k].y - cy) / focal_y,
                        1.0
                    };
                    const scalar_t ray_depth = projected_depth[k];

                    const vec3 normal = {
                        view2gaussian[0] * ray_k.x +
                            view2gaussian[1] * ray_k.y + view2gaussian[2],
                        view2gaussian[1] * ray_k.x +
                            view2gaussian[3] * ray_k.y + view2gaussian[4],
                        view2gaussian[2] * ray_k.x +
                            view2gaussian[4] * ray_k.y + view2gaussian[5]
                    };
                    ...
                    scalar_t power =
                        -0.5f * (AA * depth * depth + BB * depth + CC);
                    if (power > 0.0f) {
                        power = 0.0f;
                    }
                    ...
                    const scalar_t next_T = point_Ts[k] * (1.0f - alpha);
                    const scalar_t vis = alpha * point_Ts[k];
                    point_alphas[k] += vis;
                    point_Ts[k] = next_T;
                }
            }
        ...
        if (inside) {
            for (int k = 0; k < num_projected; k++) {
                ...
                // // write colors
                for (uint32_t ch = 0; ch < CDIM; ++ch) {
                    out_integrated_colors[3 * projected_ids[k] + ch] =
                        backgrounds == nullptr
                            ? pix_out[ch]
                            : (pix_out[ch] + corner_Ts[0] * backgrounds[ch]);
                }
            }
        }
    }
...
std::tuple<at::Tensor, at::Tensor> call_kernel_with_dim(
    // Point parameters
    ...
    const at::Tensor view2gaussians, // [C, N, 10] or [nnz, 10]
    ...
    // + 1 for depth and + 3 for normal
    at::Tensor renders = at::zeros(
        {C, image_height, image_width, channels + 1 + 3},
        means2d.options().dtype(at::kFloat)
    );
    at::Tensor alphas = at::empty(
        {C, image_height, image_width, 1}, means2d.options().dtype(at::kFloat)
    );
    at::Tensor out_integrated_alphas =
        at::full({C, PN}, 1.0, means2d.options().dtype(at::kFloat));
    at::Tensor out_integrated_colors =
        at::full({C, PN, 3}, 0.0, means2d.options().dtype(at::kFloat));
    // printf(
    //     "out_integrated_alphas size: %d %d\n",
    //     out_integrated_alphas.size(0),
    //     out_integrated_alphas.size(1)
    // );
    // 1 for last_ids and 1 for max_contributor
    at::Tensor last_ids = at::empty(
        {C, image_height, image_width, 2}, means2d.options().dtype(at::kInt)
    );

    at::cuda::CUDAStream stream = at::cuda::getCurrentCUDAStream();
    const uint32_t shared_mem = tile_size * tile_size *
                                (sizeof(int32_t) + sizeof(vec3) + sizeof(vec3) +
                                 sizeof(vec10) + sizeof(vec3));
    ...
    integrate_to_points_fwd_kernel<CDIM, float>
        <<<blocks, threads, shared_mem, stream>>>(
            ...
            point_n_isects,
            packed,
            reinterpret_cast<vec2 *>(points2d.data_ptr<float>()),
            ...
            reinterpret_cast<vec10 *>(view2gaussians.data_ptr<float>()),
            Ks.data_ptr<float>(),
            ...
            point_tile_offsets.data_ptr<int32_t>(),
            point_flatten_ids.data_ptr<int32_t>(),
    ...
    return std::make_tuple(out_integrated_colors, out_integrated_alphas);
}
