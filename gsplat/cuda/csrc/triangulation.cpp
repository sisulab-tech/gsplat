...
std::vector<uint4> triangulate(size_t num_points, float3 *points) {
    ...
    for (size_t i = 0; i < num_points; i++) {
        const auto p = Point(points[i].x, points[i].y, points[i].z);
...
std::shared_ptr<OctreeNode> build_octree(
    const std::vector<std::pair<Point, unsigned int>> &points,
    float min_x,
    float max_x,
    float min_y,
    float max_y,
    ...
    for (const auto &p : points) {
        int index = ((p.first.x() >= mid_x) << 2) |
                    ((p.first.y() >= mid_y) << 1) | (p.first.z() >= mid_z);
...
at::Tensor py_triangulate(const at::Tensor &points) {
    TORCH_CHECK(
        ...
        "points must have shape [num_points, 3]"
    );
    const auto points_ = points.cpu().contiguous();

    std::vector<uint4> cells = triangulate(
        points_.size(0), reinterpret_cast<float3 *>(points_.data_ptr())
    );

    if (cells.size() >= (size_t)std::numeric_limits<int>::max) {
        throw std::runtime_error("Too many points!");
    }
    auto cells_out = at::empty(
        {(long)cells.size(), 4}, at::dtype(at::kInt).device(at::kCPU)
    );
    memcpy(
        cells_out.data_ptr(),
