...
__global__ void relocation_kernel(
    ...
    for (int i = 1; i <= n_idx; ++i) {
        for (int k = 0; k <= (i - 1); ++k) {
            ...
            // Integer (-1)^k via branch instead of pow(-1.0f, k): under
            // --use_fast_math, powf collapses to exp2f(y*log2f(x)) and
            // log2f of a negative base is NaN, poisoning every term and
            // ultimately new_scales. See upstream gsplat PR #953.
