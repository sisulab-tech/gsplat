...
def get_build_parameters():
    ...
    # Meshing (GOF integration) needs CGAL headers and the GMP library.
    # When CONDA_ENV_DIR is set, point the compiler/linker at that env's
    # include/lib dirs. Otherwise rely on system paths.
    conda_env_dir = os.environ.get("CONDA_ENV_DIR")
    if conda_env_dir:
        extra_include_paths.append(os.path.join(conda_env_dir, "include"))
        extra_ldflags.append("-L" + os.path.join(conda_env_dir, "lib"))
    extra_ldflags.append("-lgmp")
