...
def project_points(
    ...
    radii, means2d, depths = _make_lazy_cuda_func("project_points_fwd")(
        means, viewmats, Ks, width, height, near_plane, far_plane
    )
...
def integrate_to_points(
    ...
    render_colors, render_alphas = _make_lazy_cuda_func("integrate_to_points_fwd")(
        points2d,
        point_depths,
        means2d,
        conics,
...
def view_to_gaussians(
    ...
    assert quats.size() == (N, 4), quats.size()
    assert scales.size() == (N, 3), scales.size()
    assert radii.size() == (C, N, 2), radii.size()
    means = means.contiguous()
