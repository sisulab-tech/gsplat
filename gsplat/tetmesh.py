...
def _unbatched_marching_tetrahedra(vertices, tets, sdf, scales):
    ...
    if tets.shape[0] > chunk_size:
        ...
        for tet_chunk in tqdm.tqdm(torch.chunk(tets, tets.shape[0] // chunk_size + 1)):
            ...
            if merged_verts is None:
                ...
                # merge vertices
                unique_verts_0 = torch.zeros(
                    (unique_edges.shape[0], 2, 3), dtype=torch.float, device=device
                )
                unique_verts_1 = torch.zeros(
                    (unique_edges.shape[0], 2, 1), dtype=torch.float, device=device
                )
                ...
                unique_faces_1 = idx_map[
                    faces.reshape(-1) + merged_verts[0].shape[0]
                ].reshape(-1, 3)
